...
describe CertificateVerification do
  ...
  describe "production environment" do
    it "should just return true" do
      subject.verified_organization_certificate?.should be_true
    end
  end

  describe "any other environment but production" do
    before(:each) do
      Rails.env.stub :production? => true
      
      subject.stub :organization => mock_organization
      subject.stub :certificate => 'CERTIFICAAT'
      
      @trusted_certificate = mock(OpenSSL::X509::Certificate)
      @incoming_certificate = mock(OpenSSL::X509::Certificate)
      
      mock_organization.stub_chain(:certificate, :certificate).and_return(@trusted_certificate)
      OpenSSL::X509::Certificate.stub :new => @incoming_certificate
    end
    
    it "should just return true" do
      @trusted_certificate.should_receive(:==).with(@incoming_certificate).and_return true
      subject.verified_organization_certificate?.should be_true
    end
  end
